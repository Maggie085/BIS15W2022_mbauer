...
```r
ggplot(data=iris, mapping=aes(x=Sepal.Width, y=Petal.Length))+ geom_point()
```
...
## # … with 15 more variables: mean.mass.g <int>, log10.mass <int>,
...
Positive relationship and NAs are left out 
to remove NA 
...
### Bar Plot: `geom_col()`- produces the same graph just gives names instead of #
...
can also use select to narrow down variables
...
```r
homrange_mammals <- filter(homerange, taxon == "mammals")
homrange_mammals
```
