...
Chop up data frames to focus on question of interest 
## `select()`
...
```
end up with a data frame that only has those variables 
...
## # … with 4,023 more rows
```
variables from fish_id to length
...
## [5] "radii_length_mm" "scalelength"
```
Only see colums that contain the word length
...
look for where it is numeric <dbl>- a particular class of data
...
##  Max.   :   7.500
```
-999=NA- don't ever record NA as a number 
uncommon for there to be no NAs at all 
