...
## # … with 37 more rows
```
R will interpret years as numeric 
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "sienna3", bins=40)+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="olivedrab", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "sienna3", bins=40)+
  labs(title = "Distribution of Homerange Area")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), fill = "sienna3", alpha = 0.4, color = "black")+
...
## $ prey.size.reference        <chr> NA, NA, NA, NA, NA, NA, "Brose U, et al. 20…
## $ mass_category              <chr> "large", "medium", "small", "small", "small…
## $ range_category             <chr> "medium", "small", "small", "small", "small…
```
...
## # A tibble: 22 × 3
## # Groups:   taxon [9]
##    taxon       range_category     n
##    <chr>       <chr>          <int>
##  1 birds       large             47
##  2 birds       medium            80
##  3 birds       small             13
##  4 lake fishes medium             8
##  5 lake fishes small              1
##  6 lizards     medium             7
##  7 lizards     small              4
##  8 mammals     large             84
##  9 mammals     medium           108
## 10 mammals     small             46
## # … with 12 more rows
```
