...
```{r}
top_speed <- c(venome_GT,chiron,veyron)
mean(top_speed)
```
...
There is an NA in the data set so you cannot get a numerical value for the mean. An NA within a data set prevents you from calculating certain functions like mean. 
...
```
By removing the NA you are left with a regular set of numbers that you can take the means of. 
