...
```r
good_guys <- subset(superhero_info, alignment == "good")
good_guys
```
...
```r
bad_guys <- subset(superhero_info, alignment == "bad")
bad_guys
```
...
## # … with 1 more variable: weight <dbl>
```
#We don't have 16 rows because some of the superheros fit both categories so there is some overlap. 
...
##   <chr>      <lgl>   <lgl>               <lgl>              <lgl>               
## 1 Alien      TRUE    FALSE               FALSE              FALSE               
...
```
#agility, cold resistance, durability, stealth, super strength, super speed, enhanced senses, jump, substance secretion, nautral armor, heat resistance, natural weapons, wallcrawling, vision thermal 
